...
- `mcp_integration/` — подключение к MCP-серверам (Model Context Protocol). Два
  ...
  вызывает найденные инструменты — только перечисляет их), и доступ основного ответа
  `/smart_agent` к инструментам рыночных данных (`market_session.py`), где инструменты
  как раз вызывает модель. Серверы у этих двух применений разные и настраиваются
  независимо (`MCP_SERVER_COMMAND`/`MCP_SERVER_ARGS` против `MCP_MOEX_DIR`).
...
`config.py` требует окружения бота.
...
- **Только инструменты чтения.** Модели отдаются лишь инструменты, которые сервер сам
  пометил `read_only_hint=True` (`is_read_only`); отсутствие аннотации — НЕ read-only
  (в протоколе инструмент по умолчанию считается потенциально изменяющим). Вызов
  инструмента вне этого списка модель получает как ошибку, до сервера он не доходит.
...
- Никаких админ/владельческих команд, никакого переключения модели или провайдера во
  ...
  ослаблять обязательные предупреждения `SYSTEM_PROMPT`.
