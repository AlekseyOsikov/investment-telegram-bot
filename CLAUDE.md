...
- `agents/` — агенты: сущности, инкапсулирующие цикл «вопрос пользователя -> вызов
  ...
  - `agents/task_state.py` — правила конечного автомата рабочей задачи
    ...
    `validation` → `execution`), условия перехода, разбор и валидация JSON-ответа
    технического вызова, а также все тексты про состояние (контекст для LLM,
...
только пользователь явной командой».
...
2. **Проверка результата рабочей задачи КОДОМ.** Как только автомат принял результат
   рабочего этапа и ушёл на `validation`, `SmartAgent._check_invariants()` делает
...
`data` и текстами инструкций:

| Сценарий | `planning` собирает | `execution` заполняет |
|---|---|---|
| `portfolio` — составление портфеля | `goal_type`, `horizon`, `risk`, `constraints` | `allocation` (доли в %) |
| `asset` — разбор актива/идеи | `asset`, `horizon`, `role_in_portfolio` | `analysis` |
| `review` — ревизия портфеля | `current_allocation` (только доли), `concern`, `constraints` | `proposed_changes` |

Код автомата про конкретные сценарии ничего не знает — новый сценарий добавляется
одной записью в `SCENARIOS`, без изменений в логике переходов.

**Условие перехода — это список обязательных ключей `data` текущего этапа
(`TaskStage.required_keys`), и проверяет его КОД (`task_state.missing_keys`/
`_forward_target`), а не модель.** Тот же список недостающих ключей уходит и в
...
сценария (`task_state.normalize_data_updates`), остальные отбрасываются — иначе
модель на каждом ходу изобретала бы новое имя для того же параметра
(`horizon`/«горизонт»/`time_horizon`), условие перехода никогда бы не выполнилось и
задача зависла бы навсегда. На РУЧНУЮ запись (`/smart_agent_task_set`) фильтр не
распространяется: там ключ задаёт человек. `verdict` дополнительно ограничен двумя
значениями (`accepted`/`changes_requested`) — от него зависит и сам факт завершения
задачи, и направление отката с этапа проверки, поэтому произвольный текст вроде
«пользователь вроде согласен» отбрасывается, а не трактуется на глаз.
...
прежде чем бот согласится начать новую работу.
...
(старт, смена этапа, архивирование) — только когда событие произошло.
`/smart_agent_task_stage <этап>` переводит этап вручную, если автомат ошибся или
застрял: граф переходов при этом соблюдается, но заполненность обязательных ключей
НЕ проверяется — иначе застрявшую задачу нельзя было бы сдвинуть. Отдельного
тумблера «выключить автоматику» нет: выключение слоя `working`
