...
- `agents/` — агенты: сущности, инкапсулирующие цикл «вопрос пользователя -> вызов
  ...
  - `agents/market_tools.py` — правила и тексты слоя `tools` `/smart_agent` (доступ
    основного ответа к инструментам MCP-сервера рыночных данных `mcp-moex`): три
    варианта системного сообщения слоя (сервер доступен / недоступен / слой выключен), запись о
    вызове (`ToolCallRecord`) и строки для чата (`🔧 …`, предупреждения), статусы для
    `/smart_agent_show` и сам ЦИКЛ вызовов инструментов `run_tool_loop` с
    внедряемыми `complete`/`call_tool` — он не знает ни про `openai`, ни про MCP,
    поэтому все его ветвления проверяются тестами на простых фейках
    (`tests/test_market_tools.py`). Отделён от `agents/smart_agent.py` по тому же
    принципу, что `agents/invariants.py` и `agents/task_state.py`: здесь правила и
    тексты, там — владение состоянием, настройка слоя и вызовы LLM. Про механизм
    целиком см. «Инструменты рыночных данных smart-агента» в разделе «Архитектура».
  - `agents/smart_agent_command.py` — команда `/smart_agent` (`ConversationHandler`,
    ...
    `/smart_agent_show` (профиль + инварианты + все три слоя как есть + статус слоя
    `tools` и его последние вызовы + последний собранный контекст LLM) и
    `/smart_agent_toggle` (включить/выключить слой, включая профиль, инварианты и
    `tools`, в контексте без удаления данных) и `/smart_agent_reset`
...
- `mcp_integration/` — подключение к MCP-серверам (Model Context Protocol). Три
  независимых применения: `/mcp_tools` — технический/диагностический режим по тому же
  принципу, что `research/*` (не используется в обычном потоке сообщений и НЕ
  вызывает найденные инструменты — только перечисляет их); доступ основного ответа
  `/smart_agent` к инструментам рыночных данных (`market_session.py`), где инструменты
  как раз вызывает модель; и обращения команд опроса цен `/watch*` к инструментам
  расписания (`watch_session.py`), без участия модели. Сервер у `/mcp_tools` свой и
  настраивается независимо (`MCP_SERVER_COMMAND`/`MCP_SERVER_ARGS`), у двух других
  один и тот же проект `mcp-moex` (`MCP_MOEX_DIR`), но разные режимы запуска: для
  модели без `--watch-db`, для опроса цен с ним.
  ...
  - `mcp_integration/market_session.py` — сессия с MCP-сервером рыночных данных для
    основного ответа `/smart_agent`. В отличие от `client.py` сессия открыта на весь
    ВОПРОС пользователя (`open_market_tools()` — асинхронный контекстный менеджер),
    процесс сервера один на вопрос и не живёт между вопросами. Команда запуска
    (`uv run --directory <MCP_MOEX_DIR> mcp-moex`) собирается списком аргументов без
    shell. Неудача ЗАПУСКА сервера — исключение, которое перехватывает вызывающий код
    (`SmartAgent`); неудача ВЫЗОВА инструмента (неизвестное имя, невалидные
    аргументы, ошибка сервера, обрыв обмена, тайм-аут) исключением не становится:
    `MarketTools.call()` возвращает текст для модели (`ToolOutcome`). Чистые функции
    (фильтр read-only по аннотации сервера, схема инструмента для OpenAI, разбор
    аргументов, текст результата, усечение) покрыты `tests/test_market_tools.py`.
    Модели отдаётся инструмент только если он И помечен read-only, И входит в явный
    перечень `MODEL_TOOL_ALLOWLIST` (поиск, цена, история) — второй рубеж защиты от
    инструментов расписания (`is_model_tool`); сборка команды запуска вынесена в
    `build_server_params` и намеренно НЕ содержит `--watch-db`.
  - `mcp_integration/watch_session.py` — обращения к инструментам РАСПИСАНИЯ того же
    сервера `mcp-moex` (режим `--watch-db`) для `price_watch/`: один процесс на одно
    обращение (`call_watch_tool`: запуск → рукопожатие → один вызов → закрытие), без
    удержания между обращениями. Отдельно от `market_session.py`: инструменты
    расписания принимают `chat_id` от клиента, модель их не видит. Путь к файлу
    хранилища передаётся АБСОЛЮТНЫМ (`uv run --directory` меняет рабочий каталог
    процесса, относительный путь указал бы внутрь каталога `mcp-moex`).
    `WatchToolError` — отказ инструмента с русским текстом сервера (английский
    префикс SDK «Error executing tool …» снимается); исключения запуска и обмена не
    перехватываются, `ExceptionGroup` из одного исключения разворачивается (та же
    причина, что в `SmartAgent._tool_completion_async`). Чистые функции покрыты
    `tests/test_watch_session.py`.
...
- `price_watch/` — опрос цен по тикерам с регулярными сводками (`/watch*`): бот
  задаёт, а сервер `mcp-moex` (режим `--watch-db`) хранит замеры и считает агрегаты
  и текст сводки; часы и доставка — здесь. Не привязан к `RESEARCH`, активен по
  `PRICE_WATCH_ACTIVE` (см. «Конфигурация»). Про механизм целиком см. «Опрос цен и
  сводки по расписанию» в разделе «Архитектура».
  - `price_watch/parse.py` — чистый разбор аргументов `/watch`: тикеры (через пробелы и/или
    запятые), два последних слова — периоды (`m/h/d` или `м/ч/д`, регистр не важен,
    русские единицы переводятся в латинские). Проверяет только ФОРМУ (в том числе
    вид тикера); границы периодов и существование бумаги проверяет сервер.
  - `price_watch/texts.py` — чистые тексты команд по ответам сервера (времена по МСК из
    ISO-строк сервера, склонение «раз в N минут», подсказка формата) и
    `friendly_tool_error` (сырой английский дамп валидатора схемы заменяется
    русской фразой). Текст самой СВОДКИ собирает сервер, бот шлёт его без правок.
  - `price_watch/scheduler.py` — `run_tick()` (один тик: выдача, отправка, остановка
    недоступных чатов, подтверждение; вызов сервера и отправка внедряются) и
    `WatchScheduler` (цикл ожидания на `asyncio`, без JobQueue/APScheduler);
    `start_scheduler`/`stop_scheduler` — хуки `post_init`/`post_stop` приложения.
    Покрыт `tests/test_price_watch_scheduler.py` на фейках, без сна и сети.
  - `price_watch/commands.py` — обработчики `/watch`, `/watch_status`, `/watch_report`,
...
`tests/test_market_tools.py` — тот же принцип для слоя `tools` `/smart_agent`:
чистые функции `mcp_integration/market_session.py` (на настоящих типах `mcp.types`,
собранных вручную) и `agents/market_tools.py`, включая цикл вызовов на фейковых
`complete`/`call_tool` — без сети, без запуска процессов и без моков `SmartAgent`.
Запуск сервера и живой обмен проверяются вручную (см. «Инструменты рыночных данных
smart-агента»); класс `SmartAgent` тестами не покрывается, т.к. импорт
`config.py` требует окружения бота. Тот же принцип у опроса цен:
`tests/test_price_watch_parse.py`/`tests/test_price_watch_texts.py` (чистые разбор и
тексты), `tests/test_watch_session.py` (параметры запуска, разбор результата,
группы исключений) и `tests/test_price_watch_scheduler.py` (тик и цикл планировщика на
...
`MCP_MOEX_DIR` (`config.py`, по умолчанию пусто) — каталог проекта локального
MCP-сервера рыночных данных `mcp-moex`, к которому обращается основной ответ
`/smart_agent` (слой `tools`, см. «Инструменты рыночных данных smart-агента»). Пусто —
возможность неактивна и молчит. Как и `MCP_SERVER_COMMAND`, это настройка ОПЕРАТОРА:
пользователь чата сервер не выбирает. Команда запуска (`uv run --directory <каталог>
mcp-moex`) собирается в `mcp_integration/market_session.py` списком аргументов, а не
берётся из окружения: вынесен в настройку именно каталог. Если переменная задана, но
каталога нет или в нём нет `pyproject.toml`, `config.py` пишет предупреждение и
НЕ останавливает процесс (`_warn_if_moex_dir_invalid`) — это необязательная
возможность, а не обязательный ключ. `MCP_MAX_TOOL_STEPS` (по умолчанию 5) — сколько
раз за один вопрос модель может запросить вызовы инструментов до принудительного
ответа без них; `MCP_TOOL_RESULT_MAX_CHARS` (по умолчанию 45000) — предел размера
результата инструмента, возвращаемого модели: выбран так, чтобы ПОЛНЫЙ ответ
`get_price_history` (300 свечей, ≈42 300 символов в компактном JSON) умещался — предел
не должен срабатывать штатно (обрезка идёт с головы, а свечи хронологические).
Оба числа проверяются в `_validate_config()`. Тайм-аут запуска и каждого вызова —
общий `MCP_TIMEOUT_SECONDS`. Слой не привязан к `RESEARCH`: это часть основного
ответа `/smart_agent`, а не исследовательская команда.

`PRICE_WATCH`/`PRICE_WATCH_DB`/`PRICE_WATCH_TIMEOUT_SECONDS` (`config.py`) — опрос цен
по тикерам с регулярными сводками (`/watch*`, `price_watch/`, см. «Опрос цен и сводки
по расписанию» в разделе «Архитектура»). `PRICE_WATCH` — булев флаг по образцу
`RESEARCH` (по умолчанию `true`, значения и проверка в `_validate_config()`); итоговая
`PRICE_WATCH_ACTIVE = PRICE_WATCH and bool(MCP_MOEX_DIR)` — то есть возможность
использует ТОТ ЖЕ каталог сервера, что слой `tools` smart-агента, и без `MCP_MOEX_DIR`
неактивна и молчит (команды не регистрируются, в `/help` и `/start` не упоминаются).
Отдельный флаг нужен, чтобы оператор мог оставить smart-агенту данные биржи, но не
включать бота, который пишет сам; как и `RESEARCH`, это решение оператора, а не
пользователя чата. `PRICE_WATCH_DB` (по умолчанию `data/price_watch/watch.sqlite3`) —
файл SQLite, который ведёт сервер; путь приводится к АБСОЛЮТНОМУ при загрузке
конфигурации, потому что `uv run --directory` меняет рабочий каталог процесса
сервера (относительный путь указал бы внутрь каталога `mcp-moex`). Каталог `data/` в
`.gitignore`. `PRICE_WATCH_TIMEOUT_SECONDS` (по умолчанию 90, должен быть > 0) — предел
одного обращения к серверу расписания (запуск + рукопожатие + вызов); больше
`MCP_TIMEOUT_SECONDS`, потому что плановый вызов опрашивает биржу по всем чатам разом.
...
"working": bool, "long_term": bool, "tools": bool}`, все шесть `True` по умолчанию;
файлы без ключа `tools` читаются как «включён» — `_load_state()` стартует с
«все включены» и накладывает сохранённое, отдельная миграция не нужна) — ОБЩАЯ НА ВЕСЬ
...
#### Инструменты рыночных данных smart-агента

Слой `tools` (`LAYER_TOOLS`) даёт ОСНОВНОМУ ответу `/smart_agent` доступ к
инструментам локального MCP-сервера `mcp-moex` (поиск бумаги Московской биржи, текущая
цена, история цен) через function calling: модель видит инструменты, сама решает,
вызывать ли их, получает результаты и продолжает до финального ответа. Это не слой
памяти и не ограничение, а ВОЗМОЖНОСТЬ — он ничего не хранит и ничего не запрещает,
поэтому, в отличие от инвариантов, его выключение не требует напоминания ПОЛЬЗОВАТЕЛЮ
каждый ход (о выключении сообщает ответ `/smart_agent_toggle`). Модели же сказать о
выключении нужно — см. «Слой выключен» ниже.
Правила и тексты — `agents/market_tools.py`, механика подключения —
`mcp_integration/market_session.py`, владение слоем и вызовы LLM — `SmartAgent`.

Когда слой активен: включён (`/smart_agent_toggle tools`) И задан `MCP_MOEX_DIR`.
Иначе `ask()` идёт прежним путём — один синхронный вызов без ключа `tools`, без
запуска процесса и без `asyncio`; поведение и стоимость чата без слоя не изменились —
единственное отличие: при ВЫКЛЮЧЕННОМ слое и заданном `MCP_MOEX_DIR` в контексте одно
короткое сообщение (см. «Слой выключен»).

Как это устроено, и что нужно сохранять при доработке:
- **Один процесс сервера на ВОПРОС.** `SmartAgent.ask()` (остаётся синхронным) в пути
  с инструментами вызывает `asyncio.run(_tool_completion_async())`: сессия открывается
  в начале вопроса (схемы нужны до первого обращения к модели), живёт на все вызовы
  этого вопроса и закрывается в конце. Цена — ≈0,75 с на каждый вопрос при активном
  слое. Ленивый запуск с кэшем схем рассмотрен и отложен (design.md изменения
  `add-smart-agent-moex-tools`).
- **Мост sync/async.** Командный слой вызывает `await asyncio.to_thread(agent.ask,
  ...)`: `asyncio.run()` не работает в потоке с работающим циклом, а заодно основной
  цикл не блокируется на запросе к LLM. Потокобезопасность держится на том, что
  обновления PTB обрабатываются ПОСЛЕДОВАТЕЛЬНО (`concurrent_updates` в `main.py` не
  включён) — если это изменится, понадобится блокировка на чат. Остальные синхронные
  вызовы LLM (автомат задачи, детектор старта, ревизор) по-прежнему выполняются прямо
  в обработчике.
- **Исключения основного вызова модели должны дойти до командного слоя КАК ЕСТЬ.**
  Контексты `stdio_client`/`ClientSession` построены на `anyio`, который может завернуть
  исключение из тела в `ExceptionGroup` и обойти `except AuthenticationError` и т.п. в
  `smart_agent_receive_question` (пользователь увидел бы «непредвиденную ошибку» вместо
  понятного текста). Поэтому `_tool_completion_async` ловит ошибку тела ВНУТРИ сессии,
  запоминает и возбуждает заново уже после выхода из неё; сбой самого закрытия сессии
  при готовом ответе ответ не отменяет — только журнал. Не упрощай это до голого
  `async with`.
- **Цикл (`market_tools.run_tool_loop`)**: до `MCP_MAX_TOOL_STEPS` шагов; за один шаг
  модель может запросить несколько вызовов, и КАЖДЫЙ получает своё сообщение `tool`
  (иначе API отклонит следующий запрос). Исчерпав лимит, цикл делает ещё одно
  обращение БЕЗ `tools` с указанием ответить по имеющимся данным и помечает
  `step_limit_reached` — вопрос не остаётся без ответа. `reasoning_content` из
  сообщения ассистента возвращается в историю цикла, если он есть: спайк на DeepSeek
  показал, что цикл работает и без него (на шагах с вызовом он не приходил), поэтому это
  страховка, которую проверить не удалось.
- **Только инструменты чтения из явного перечня.** Модели отдаются лишь инструменты,
  которые сервер сам пометил `read_only_hint=True` (`is_read_only`) И которые входят в
  `MODEL_TOOL_ALLOWLIST` (`is_model_tool`: поиск, цена, история); отсутствие аннотации —
  НЕ read-only (в протоколе инструмент по умолчанию считается потенциально
  изменяющим). Вызов инструмента вне этого списка модель получает как ошибку, до
  сервера он не доходит. Перечень — второй рубеж: инструментов расписания в этой
  сессии и так нет (сервер запускается без `--watch-db`), но новый read-only
  инструмент сервера для модели теперь нужно добавить в перечень явно — это желаемое
  трение, а пропущенные инструменты называются в предупреждении журнала.
- **Ошибки инструмента — текст для модели, а не исключение.** Неизвестное имя,
  невалидный JSON в `arguments`, `is_error` от сервера (его текст уже написан для LLM
  и передаётся как есть), обрыв обмена, тайм-аут вызова — всё это содержимое сообщения
  `tool`. Только сбой ОБМЕНА (обрыв, тайм-аут) помечается `transport_failure` и даёт
  пользователю предупреждение «часть данных биржи получить не удалось»; неизвестный
  тикер — рабочая ситуация, о ней модель скажет сама.
- **Результат инструмента для модели** — компактный JSON из `structured_content`
  (без отступов: pretty-print сервера был на треть длиннее), иначе текст блоков; затем
  усечение до `MCP_TOOL_RESULT_MAX_CHARS` С ГОЛОВЫ (сводка в начале) вместе с пометкой,
  которая влезает в предел и направляет модель к меньшему периоду/более крупному
  `interval`. Предел подобран так, чтобы полный ответ сервера умещался (см. «Конфигурация»):
  обрезка не должна срабатывать штатно, иначе пропадут самые свежие свечи.
- **Недоступный сервер не блокирует ответ.** Не запустился (`uv` не найден, нет
  каталога, тайм-аут рукопожатия, сбой протокола, у сервера нет read-only-инструментов) —
  `ask()` отвечает БЕЗ инструментов, в контекст вместо обычного сообщения слоя уходит
  вариант «ДАННЫЕ БИРЖИ СЕЙЧАС НЕДОСТУПНЫ» (запрет называть текущие цены по памяти —
  без него модель ответила бы устаревшей ценой уверенным тоном, ради чего слой и
  существует), а пользователю — предупреждение. Следующий вопрос снова пытается
  подключиться. Этот же принцип, что у служебных вызовов: сбой вспомогательной части не
  роняет ответ.
- **Слой выключен, сервер настроен — модели об этом говорится.** В живом прогоне при
  выключенном слое модель повторила цену из прежнего ответа (она осталась в
  `short_term`) как «текущую» и приписала ей выдуманное время котировки. Поэтому при
  выключенном слое И заданном `MCP_MOEX_DIR` `ask()` добавляет вариант «ДАННЫЕ БИРЖИ
  ВЫКЛЮЧЕНЫ ПОЛЬЗОВАТЕЛЕМ» (`market_tools.build_disabled_context_message`): цены из
  прежних ответов не выдавать за текущие, не приписывать им время, сказать, что данные
  выключены и как включить. Без `MCP_MOEX_DIR` сообщения нет — там инструментов никогда
  не было. Это подсказка модели, а не гарантия: код не вычищает цены из памяти (диалог
  пользователя задним числом не меняется) и не проверяет ответ.
- **Место в контексте**: инварианты → профиль → инструменты → долговременная память →
  рабочая задача → краткосрочный диалог. Инварианты остаются первыми и прямо получают
  приоритет над сообщением слоя. Сообщение слоя обязывает модель считать результаты
  ДАННЫМИ, а не инструкциями (защита от инъекций через названия бумаг и тексты
  сервера), называть время котировки и её задержку, учитывать единицы (цена облигации —
  проценты от номинала), не подавать историю как гарантию, не пересылать в инструменты
  текст пользователя целиком и — самое важное — НЕ отменяет и не ослабляет
  обязательные правила основного `SYSTEM_PROMPT` (та же оговорка, что у профиля и
  инвариантов; при правке текста её убирать нельзя, тест это проверяет).
- **В память попадает только итог.** В `short_term` пишется одна пара «вопрос —
  итоговый ответ», сообщения `tool_calls`/`tool` не сохраняются; ответ, построенный на
  данных биржи, сохраняется как обычно (данные публичные, сумм пользователя среди них
  нет). Служебные вызовы LLM (`_call_json_api`) инструментов НЕ получают: `tools=`
  передаётся только из `SmartAgent._chat`.
- **Видимость.** После ответа в служебное сообщение уходят строки `🔧 <инструмент>
  <аргументы>` (с пометкой «— ошибка»), предупреждения слоя и статистика токенов (при
  нескольких обращениях к модели контекст/ответ — суммы по всем, с пометкой числа
  обращений). `/smart_agent_show` печатает статус слоя (включён / выключен / не
  настроен / недоступен с причиной) и вызовы последнего вопроса; статус хранится в
  памяти объекта, на диск не пишется.
- **Проверено только на DeepSeek** (`MAIN_CLIENT=deepseek`): цикл tool-calls на Kimi
  отдельно не проверялся — при переходе на него нужен свой спайк, как и для
  `json_object` в `research/constraints.py`.
- **Данные наружу**: серверу уходят только аргументы, которые сформировала модель
  (тикер, поисковый запрос до 100 символов, даты) — не история, не память, не профиль,
  не инварианты. Это осознанное расхождение с прежней формулировкой для `/mcp_tools`
  («серверу не передаётся текст пользователя»), оформленное в спеке изменёнными
  требованиями `mcp-integration`. Сервер `mcp-moex` ходит на ISS напрямую
  (`trust_env=False`), а `stdio_client` не передаёт дочернему процессу прокси-переменные
  бота — если сети хоста нужен прокси для биржи, эта схема не заработает.
- Слой не привязан к флагу `RESEARCH` и не относится к `/agent`, `/agent_compare`,
  `handle_message` и research-режимам — им инструменты не отдаются.

### Опрос цен и сводки по расписанию

`/watch <тикеры> <период опроса> <период сводки>` (пример: `/watch SBER GAZP 15m 1h`),
`/watch_status`, `/watch_report`, `/watch_stop` (`price_watch/`) — бот сам присылает
сводки по выбранным бумагам Московской биржи без вопроса от пользователя. Модель в
этих командах НЕ участвует: команды сами вызывают инструменты сервера `mcp-moex`
(`mcp_integration/watch_session.py`), а `chat_id` берут из `update.effective_chat.id`.

Разделение ответственности: СЕРВЕР (изменение `add-price-watch-scheduler` в
репозитории `mcp-moex`) хранит опросы, замеры и сводки в SQLite, опрашивает ISS по
вызову клиента, считает агрегаты и собирает текст сводки (шаблон без LLM, с оговоркой о
задержке котировок и «не рекомендация»); БОТ — команды, часы и доставка. Сервер
«пассивен»: время сам не измеряет и ничего не отправляет.

```
 /watch ------------> watch_set(chat_id, тикеры, периоды) ---> сервер (SQLite)
 WatchScheduler:  ждёт next_due_at (не дольше часа) ----> тик:
     watch_run_due -> сводки к отправке + next_due_at
     отправка в чат -> watch_ack(id доставленных)   [ТОЛЬКО после успешной отправки]
```

Что нужно сохранять при доработке (см. design.md изменения `add-price-watch-commands`):
- **Один процесс сервера на обращение**, без удержания между ними (`call_watch_tool`).
  Два обращения за тик (`watch_run_due`, затем `watch_ack`), а не одна сессия, которая
  висела бы открытой, пока бот отправляет сообщения.
- **Доставка «не менее одного раза».** Сначала отправка, потом подтверждение: сбой между
  ними даёт повторную отправку сводки, а не потерю. Сбой отправки оставляет сводку
  неподтверждённой; после неудачи в чате следующие сводки ЭТОГО чата в тике не
  отправляются (порядок), сбой одного чата не мешает остальным. «Бот заблокирован»/
  «chat not found» (`is_chat_unreachable`) — `watch_stop` для чата, без бесконечных
  повторов.
- **Планировщик на `asyncio`, без JobQueue.** `run_tick()` не знает ни про Telegram, ни
  про MCP (вызов сервера и отправка внедряются) — так всё ветвление проверяется
  тестами без сна и сети, по образцу `run_tool_loop`. Первый тик — сразу при запуске
  (доставить сводки за простой бота); ожидание не дольше `RECHECK_SECONDS` (час:
  страховка от прыжков часов); сбой сервера или доставки — пауза 60 с, удвоение до
  900 с, сброс после успеха; пользователям о фоновых сбоях не пишем. `/watch` вызывает
  `schedule_at(min(next_poll_at, next_report_at))` — срок переводится только на более
  РАННИЙ момент и без лишнего вызова сервера; `schedule_at` во время идущего тика
  учитывается по его итогам (ответ сервера мог уйти ДО команды).
- **Модель инструментов расписания не видит.** Сервер для ответов `/smart_agent`
  запускается БЕЗ `--watch-db` (`build_server_params`), плюс перечень
  `MODEL_TOOL_ALLOWLIST`. Инструменты расписания сервер помечает НЕ read-only именно
  поэтому: пометка «только чтение» в проекте означает «можно отдавать модели».
- **Бот проверяет только форму команды** (вид тикера, `число + m/h/d|м/ч/д`); границы
  периодов, число тикеров и существование бумаги — на сервере, и его русский отказ
  показывается как есть (`WatchToolError`; отказ схемы валидатора заменяется
  `friendly_tool_error`). Серверный текст «опрос не задан» написан для модели (называет
  инструмент `watch_set`), поэтому `/watch_report` для пустого чата уточняет состояние
  через `watch_status` и отвечает своими словами.
- **Текст сводки бот отправляет без правок**: оговорки сервера (задержка на 15 минут,
  «не инвестиционная рекомендация») должны доезжать до пользователя.

Проверено на живом `mcp-moex` с подменённым Telegram (постановка, отказы сервера,
статус, сводка по запросу, плановая доставка с подтверждением, остановка при блокировке
бота); сам Telegram проверяется вручную.
...
- Рыночные данные в `/smart_agent` (слой `tools`, см. «Инструменты рыночных данных
  smart-агента») не меняют доменных правил: цена или история от инструмента не делают
  ответ гарантией результата, котировка называется вместе со временем и задержкой, а
  обязательные предупреждения `SYSTEM_PROMPT` остаются в силе. Наружу уходят только
  аргументы, сформированные моделью, — персональные данные пользователя серверу не
  передаются. Расширение этого канала (передача данных пользователя серверу,
  инструменты, которые что-то изменяют, любая связь с брокером и исполнением сделок) —
  отдельное архитектурное решение, обсуждаемое явно, а не побочный эффект доработки.
- Опрос цен (`/watch*`, см. «Опрос цен и сводки по расписанию») — первый случай, когда
  бот сам пишет пользователю без вопроса. Это РЕАЛИЗОВАННОЕ, явно запрошенное
  исключение: включается только явной командой `/watch`, выключается `/watch_stop`,
  при блокировке бота опрос чата останавливается сам. Хранится только идентификатор
  чата, тикеры, настройки опроса, публичные цены и готовые сводки — не диалоги и не
  персональные финансовые данные; команды не запрашивают суммы, счета и остатки.
  Сводка — факты по ценам, а не рекомендация: оговорки сервера (задержка котировок,
  «не рекомендация») должны сохраняться в тексте, который получает пользователь.
  Любое расширение (пороги и уведомления о «сигналах», сводки с оценками и советами,
  сумма или количество бумаг пользователя, связь с брокером) — отдельное решение.
...
- Никаких админ/владельческих команд, никакого переключения модели или провайдера во
  ...
  доработке текста, иначе слой превратится в пользовательский системный промпт. Не
  относится и к слою `tools` (доступ ответа к инструментам рыночных данных, см.
  «Инструменты рыночных данных smart-агента»): сервер и его команда запуска задаёт
  оператор (`MCP_MOEX_DIR`), пользователь чата их не выбирает, модели отдаются только
  инструменты чтения, а сообщение слоя, как и у инвариантов, прямо запрещает
  ослаблять обязательные предупреждения `SYSTEM_PROMPT`. Не относится и к опросу цен
  (`/watch*`): сервер, его команду запуска и путь к файлу задаёт оператор
  (`MCP_MOEX_DIR`, `PRICE_WATCH_DB`), команды не меняют ни провайдера, ни модель, ни
  системный промпт и не используют LLM вообще, а инструменты расписания модели не
  отдаются (сервер для ответов запускается без `--watch-db`, плюс явный перечень).
...
- Опрос цен (`/watch*`, `price_watch/`) — ещё одно ЯВНО запрошенное исключение из «нет
  памяти» и «бот только отвечает», не связанное с `/agent`/`/smart_agent`: сервер
  `mcp-moex` хранит в SQLite (`PRICE_WATCH_DB`, каталог `data/` в `.gitignore`)
  идентификатор чата, тикеры, периоды, публичные цены и готовые сводки, а бот сам шлёт
  сводки по расписанию. Серверу уходят только идентификатор чата, тикеры и периоды из
  команды — не текст других сообщений, не история, не память агентов. `/agent_reset`/
  `/smart_agent_reset` эти данные не трогают, удаляет их `/watch_stop`. Не
  распространяй на этот канал сбор денежных сумм и личных финансовых данных.
