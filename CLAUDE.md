...
- `agents/` — агенты: сущности, инкапсулирующие цикл «вопрос пользователя -> вызов
  ...
  - `agents/agent.py` — класс `Agent`, использует `main_client`/`MAIN_MODEL` (как и
    ...
    «Ограничения безопасности» ниже. Сырая история хранится на диске без
    ограничений, но в LLM с каждым вопросом уходит не она целиком, а управляемый
    контекст: текстовая сводка (summary) более старой части плюс несвёрнутый
    "свежий хвост" — см. «Управление контекстом агента» в разделе «Архитектура».
...
диалога целиком — вместо этого более старая часть периодически сворачивается в
текстовую сводку (`summary`), и каждый запрос содержит `summary` (если он уже
есть) плюс "свежий хвост" — все пары вопрос-ответ, ещё не попавшие в сводку.
`AGENT_CONTEXT_RECENT_PAIRS` (см. «Конфигурация») — минимальный размер этого
...
нуля при каждом срабатывании — сохраняй этот принцип при доработке, полная
пересборка с ростом истории становится всё дороже по токенам). "Свежий хвост" —
это буквально «всё, что ещё не попало в summary», а не отдельное окно
фиксированного размера, поэтому пар, которые не входят ни в сводку, ни в
отправляемый хвост, не бывает — см. `Agent._maybe_update_summary()`.

Полная сырая история при этом продолжает храниться на диске без ограничений
(`self._history` в JSON-файле, поле `messages`) — `summary`/`summarized_pairs`
это дополнительные поля того же файла, влияющие только на то, что уходит в LLM,
а не на то, что хранится. `/agent_history` (`agent_command.py`) по-прежнему
печатает весь сырой диалог как есть, а не сокращённый контекст — сохраняй это
при доработке, а не переключай его на показ `summary`. `/agent_reset` очищает и
`summary`/`summarized_pairs` вместе с историей.

Если вызов на сворачивание сам завершится ошибкой API, это не должно ронять уже
полученный ответ пользователю на его текущий вопрос: `Agent._maybe_update_summary()`
— единственное место в `Agent`, где исключения OpenAI SDK перехватываются внутри
самого класса (в остальном `ask()` их сознательно не ловит, см. выше), — при
ошибке `summary`/`summarized_pairs` остаются прежними, а попытка свернуть тот же
блок повторяется при одном из следующих вопросов.
